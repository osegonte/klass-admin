...
export default function TopicCourseBuilder({
  assignmentId, topicId, assignmentStatus,
}: Props) {
  ...
  const isReadOnly = assignmentStatus === 'approved' || assignmentStatus === 'submitted'

  const fetchContent = useCallback(async () => {
    const supabase = createClient()
    const [{ data: blocksData }, { data: flashcardsData }] = await Promise.all([
      supabase.from('content_blocks').select('*').eq('topic_id', topicId).order('block_order'),
      supabase.from('flashcards').select('*').eq('topic_id', topicId).order('card_order'),
    ])
  ...
  const addBlock = async (type: string, afterOrder: number) => {
    ...
    for (const b of toShift) {
      await supabase
        .from('content_blocks')
        .update({ block_order: b.block_order + 1 })
        .eq('id', b.id)
    }
  ...
  const addFlashcard = async () => {
    ...
    await supabase.from('flashcards').insert({
      id, topic_id: topicId, front: '', back: '', card_order: order,
    })
  ...
  const updateFlashcard = async (updated: Flashcard) => {
    ...
    await supabase
      .from('flashcards')
      .update({ front: updated.front, back: updated.back })
      .eq('id', updated.id)
