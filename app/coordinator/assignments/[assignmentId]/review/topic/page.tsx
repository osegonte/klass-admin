      ...
      <div className="flex flex-col gap-6 mb-8">
        ...
        {blocks?.map((block: any) => (
          <ContentBlockPreview
            key={block.id}
            block={block}
            question={questions?.find(q => q.id === block.question_id)}
            flashcard={flashcards?.find(f => f.id === block.flashcard_id)}
          />
        ))}
      </div>
        ...
        <div className="mb-8">
          ...
          <div className="flex flex-col gap-2">
            {flashcards.map((card: any) => (
              <div key={card.id} className="bg-white border border-gray-200 rounded-lg p-3 grid grid-cols-2 gap-3">
                <div>
...
function ContentBlockPreview({ block, question, flashcard }: {
  block:     any
  question?: any
  flashcard?: any
}) {
  ...
  const BLOCK_COLOR: Record<string, string> = {
    definition:  'border-l-purple-400',
    explanation: 'border-l-blue-400',
    formula:     'border-l-amber-400',
    example:     'border-l-green-400',
    keypoint:    'border-l-yellow-400',
    note:        'border-l-gray-300',
    diagram:     'border-l-teal-400',
    table:       'border-l-slate-400',
  }

  return (
    <div className={`bg-white border border-gray-200 border-l-4 ${BLOCK_COLOR[block.type] ?? 'border-l-gray-200'} rounded-lg p-4`}>
      ...
      {block.type === 'question' && question && (
        <div className="mt-3 border border-gray-100 rounded p-3 bg-stone-50">
          ...
          {question.options?.length > 0 && (
            <div className="mt-2 flex flex-col gap-1">
              {question.options.map((opt: any) => (
                <p key={opt.id} className={`text-xs ${
      ...
      {block.type === 'flashcard' && flashcard && (
        <div className="mt-3 grid grid-cols-2 gap-2">
          <div className="border border-gray-100 rounded p-2 bg-stone-50">
            <p className="text-xs text-gray-400 mb-1">Front</p>
            <p className="text-sm text-gray-800">{flashcard.front}</p>
          </div>
          <div className="border border-gray-100 rounded p-2 bg-stone-50">
            <p className="text-xs text-gray-400 mb-1">Back</p>
            <p className="text-sm text-gray-800">{flashcard.back}</p>
          </div>
        </div>
      )}
    </div>
