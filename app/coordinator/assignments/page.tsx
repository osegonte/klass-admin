...
export default async function AssignmentsPage() {
  ...
  for (const status of STATUS_ORDER) {
    grouped[status] = (assignments ?? []).filter((a: any) => a.status === status)
  }

  const total     = assignments?.length ?? 0
  const needsWork = (grouped['submitted']?.length ?? 0) +
                    (grouped['needs_revision']?.length ?? 0)
    ...
    <div>
      ...
      {total === 0 && (
        <div className="border border-dashed border-gray-300 rounded-lg p-12 text-center">
          ...
          <p className="text-xs text-gray-400 mt-1">
            Go to the Syllabus to assign topics to builders.
          </p>
        </div>
      )}
            ...
            <div className="flex flex-col gap-2">
              {grouped[status]?.map((a: any) => (
                <Link
                  ...
                  className="bg-white border border-gray-200 rounded-lg px-4 py-3 flex items-center justify-between gap-3 hover:border-gray-400 transition-colors group"
                >
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium text-gray-900 truncate">
                      {a.topics?.name ?? 'Unknown topic'}
                    </p>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <span className={`text-xs px-2 py-1 rounded font-medium ${STATUS_STYLE[status]}`}>
                      {STATUS_LABEL[status]}
                    </span>
